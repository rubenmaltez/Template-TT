...
# 5. Preparar archivos
Write-Host "`n[5/7] Preparando archivos..." -ForegroundColor Yellow
...
Write-Host "`n[7/7] Creando GitHub Release..." -ForegroundColor Yellow
try {
    ...
    Write-Host "  gh release create fallo (rate limit o auth). Abriendo browser..." -ForegroundColor Red
    Start-Process "https://github.com/$repo/releases/new?tag=v$Version"
    Write-Host "  Arrastra manualmente los archivos al release:" -ForegroundColor Yellow
    Write-Host "    - $((Get-Item $msix).FullName)" -ForegroundColor Gray
    Write-Host "    - $((Get-Item $apk).FullName)" -ForegroundColor Gray
    Write-Host "    - $((Get-Item 'version.json').FullName)" -ForegroundColor Gray
}
